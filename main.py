...
async def create_person(data: PersonCreate):
    ...
    count = await db("SELECT COUNT(*) as c FROM persons WHERE is_genesis=TRUE", fetch="one")
    genesis_code = f"G{(count['c'] if count else 0) + 1}"
    row = await db("""
...
async def add_relationship(data: RelationshipCreate):
    ...
    # pg8000 doesn't support ON CONFLICT with named params easily, use upsert logic
