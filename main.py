...
app = FastAPI(title="Ashvattha", lifespan=lifespan)
...
async def create_person(data: PersonCreate):
    ...
    count = await db("SELECT COUNT(*) FROM persons WHERE is_genesis=TRUE", fetch="val")
    genesis_code = f"G{(count or 0) + 1}"
    person_id = await db("""
...
async def search_persons(q: str, limit: int = 20):
    rows = await db("""
        ...
        ORDER BY agent_researched DESC, id ASC LIMIT %s
    """, (f"%{q}%", limit))
...
async def get_person(person_id: int):
    ...
    cats = await db("""
        ...
        JOIN person_categories pc ON pc.category_id=c.id WHERE pc.person_id=%s
    """, (person_id,))
...
async def add_relationship(data: RelationshipCreate):
    existing = await db("""
        SELECT id FROM relationships WHERE child_id=%s AND parent_type=%s AND is_primary=TRUE
    ...
    if data.source_url:
        await db("INSERT INTO sources (relationship_id, url, source_type) VALUES (%s,%s,'user')",
                 (rel_id, data.source_url), "none")
